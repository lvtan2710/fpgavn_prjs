...
//  + External memory interface with rtlmem_2r1w2x
//  + Read& write done after 2 clock cycle
...
//      6 up_ra = eng_wa # eng_ra --> cpu read ok, engine read & write ok, 
//                                    anti-conflict, updo = eng_wrd
//
...
// Created On   : Sat May 27 14:57:27 2017
// History (Date, Changed By)
//
////////////////////////////////////////////////////////////////////////////////

module up_ramsta_2r1w2x
    (
     clk,
     rst_n,
...
wire                    ram_re1, ram_re2;
wire [G_ADDR-1:0]       ram_ra1, ram_ra2;
s_pl_reg #(G_ADDR+1)    pl_ram_read (clk, rst_n, {ram_re , ram_ra },
                                                 {ram_re1, ram_ra1});
s_pl_reg #(G_ADDR+1)    pl_ram_read1(clk, rst_n, {ram_re1, ram_ra1},
                                                 {ram_re2, ram_ra2});
...
wire                    ram1_re1, ram1_re2;
wire [G_ADDR-1:0]       ram1_ra1, ram1_ra2;
s_pl_reg #(G_ADDR+1)    pl_ram1_read (clk, rst_n, {ram1_re , ram1_ra },
                                                  {ram1_re1, ram1_ra1});
s_pl_reg #(G_ADDR+1)    pl_ram1_read1(clk, rst_n, {ram1_re1, ram1_ra1},
                                                  {ram1_re2, ram1_ra2});
...
wire                    nxt_anti_stage1, cur_anti_stage1;  // when current write same read delay 1, means update new value immediately
assign                  nxt_anti_stage1 = ram_we & ram_re1 & (ram_wa == ram_ra1); // clk 1
s_dff #(0)              reg_anti_stage1(clk, rst_n, nxt_anti_stage1, cur_anti_stage1);  // clk 2

wire                    anti_stage1;
assign                  anti_stage1 = cur_anti_stage1;  // clk 2

//------------------------------------------------------------------------------
wire                    wr1_conflict;

wire                    nxt_anti1_stage1, cur_anti1_stage1;  // when current write same read delay 1, means update new value immediately
assign                  nxt_anti1_stage1 = ram_we & ram1_re1 & (ram_wa == ram1_ra1); // clk 1
s_dff #(0)              reg_anti1_stage1(clk, rst_n, nxt_anti1_stage1, cur_anti1_stage1);  // clk 2

wire                    anti1_stage1;
assign                  anti1_stage1 = cur_anti1_stage1;  // clk 2
...
generate
    ...
    if ((G_FAMILY == "XILINX") & (G_TYPE == "BLOCK")) // Because only Xilinx's Block Ram already anti-conflict at stage 0
        ...
        assign          ram_rdat     = (anti_stage1 ? ram_wdat1 : imemdo1); // Reduced 1 stage logic here

        assign          wr1_conflict = 1'b0;
        assign          ram1_rdat    = (anti1_stage1 ? ram_wdat1 : imemdo2); // Reduced 1 stage logic here
        end
    else
        begin
        ...
        wire            wr_conflict_p2;
        s_dff_nclk #(2) ppl_wr_conflict(clk, rst_n, wr_conflict, wr_conflict_p2);

        wire            anti_stage0;  // when current write same current read
        assign          anti_stage0 = wr_conflict_p2;   // clk 2

        wire [G_WIDTH-1:0]      ram_wdat2;
        s_pl_reg #(G_WIDTH)     pl_ram_wdat1(clk, rst_n, ram_wdat1, ram_wdat2);
        
        assign          ram_rdat    = (anti_stage1 ? ram_wdat1 :
                                       anti_stage0 ? ram_wdat2 : imemdo1);
        //------------------------------------------------------------------------------
        assign          wr1_conflict = ram_we & ram1_re & (ram_wa == ram1_ra);

        wire            wr1_conflict_p2;
        s_dff_nclk #(2) ppl_wr1_conflict(clk, rst_n, wr1_conflict, wr1_conflict_p2);

        wire            anti1_stage0;  // when current write same current read
        assign          anti1_stage0 = wr1_conflict_p2;   // clk 2
        
        assign          ram1_rdat    = (anti1_stage1 ? ram_wdat1 :
                                        anti1_stage0 ? ram_wdat2 : imemdo2);
...
s_pl_reg #(2)           ppl_cpu_access_ok1(clk, rst_n, {cpu_access_ok   , cpu_access_ok_p1},
                                                       {cpu_access_ok_p1, cpu_access_ok_p2});
